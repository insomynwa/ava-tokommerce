...
class Ava_Tokommerce_Ongkir_API {
	...
	private function populate_accounts() {
		$files = glob( AVA_TOKOMMERCE_PATH . 'includes/ongkir/accounts/class-ava-tokommerce-account-*.php' );

		foreach ( $files as $file ) {
			$class_name = str_replace( array( 'class-', '-' ), array( '', '_' ), basename( $file, '.php' ) );

			// var_dump(class_exists( $class_name ));die;
		...
		if ( $this->accounts ) {
			uasort( $this->accounts, 'ava_tokommerce_sort_by_priority' );
		}
	}
	...
	private function populate_couriers() {
		$files = glob( AVA_TOKOMMERCE_PATH . 'includes/ongkir/couriers/class-ava-tokommerce-courier-*.php' );
		...
		if ( $this->couriers ) {
			uasort( $this->couriers, 'ava_tokommerce_sort_by_priority' );
		}
	}
	 ...
	 * @param bool   $as_array Wethere to return data as array or not.
	 *
	 * @return (Ava_Tokommerce_Account|array|bool) Courier object or array data. False on failure.
	 */
	 ...
	 * @param bool   $as_array Wethere to return data as array or not.
	 *
	 * @return (Ava_Tokommerce_Courier|array|bool) Courier object or array data. False on failure.
	 */
	...
	public function api_response_parser( $response ) {
		try {
			...
			if ( empty( $body ) ) {
				throw new Exception( __( 'API response is empty.', 'ava-tokommerce' ) );
			}
			...
			if ( $json_error && strtolower( $json_error ) !== 'no error' ) {
				...
				throw new Exception( wp_sprintf( __( 'Failed to decode the JSON data: Error: %1$s, Body: %2$s', 'ava-tokommerce' ), $json_error, $body ) );
			}
			...
			if ( isset( $json_data['rajaongkir']['status'] ) && 200 !== intval( $json_data['rajaongkir']['status']['code'] ) ) {
				...
				throw new Exception( wp_sprintf( __( 'Code: %1$s, Description: %2$s', 'ava-tokommerce' ), $error_code, $error_description ) );
			}
			...
			throw new Exception( wp_sprintf( __( 'API response is invalid:  %1$s', 'ava-tokommerce' ), $body ) );
		} catch ( Exception $e ) {
			wc_get_logger()->log( 'error', wp_strip_all_tags( $e->getMessage(), true ), array( 'source' => 'ava_tokommerce_api_error' ) );

			// translators: %s - Error message from RajaOngkir.com.
			return new WP_Error( 'invalid_api_response', wp_sprintf( __( '<strong>Error from RajaOngkir.com</strong>: %s', 'ava-tokommerce' ), $e->getMessage() ) );
		}
	}
	...
	public function api_request_post( $endpoint = '', $body = array(), $custom_params = array() ) {
		 ...
		 * @param array        $body Body     API request parameters.
		 * @param array        $custom_params Custom API request parameters.
		 * @param Ava_Tokommerce_Ongkir_API $object        Current class object.
		 *
		 * @return bool
		 */
		$response = apply_filters( 'ava_tokommerce_api_request_post_pre', false, $endpoint, $body, $custom_params, $this );
	...
	public function api_request_get( $endpoint = '', $query_string = array(), $custom_params = array() ) {
		 ...
		 * @return bool
		 */
		$response = apply_filters( 'ava_tokommerce_api_request_get_pre', false, $endpoint, $query_string, $custom_params, $this );
			...
			foreach ( $parsed_response['results'] as $result ) {
				...
				if ( ! $courier ) {
					...
					wc_get_logger()->log(
						...
						array( 'source' => 'ava_tokommerce_api_unregistered_domestic_courier' )
					);
				...
				foreach ( $result['costs'] as $rate ) {
					...
					if ( ! isset( $courier_services[ $rate['service'] ] ) ) {
						...
						wc_get_logger()->log(
							...
							array( 'source' => 'ava_tokommerce_api_unregistered_domestic_service' )
						);
					}
			...
			foreach ( $parsed_response['results'] as $result ) {
				...
				if ( ! $courier ) {
					...
					wc_get_logger()->log(
						...
						array( 'source' => 'ava_tokommerce_api_unregistered_international_courier' )
					);
				...
				foreach ( $result['costs'] as $rate ) {
					...
					if ( ! isset( $courier_services[ $rate['service'] ] ) ) {
						...
						wc_get_logger()->log(
							...
							array( 'source' => 'ava_tokommerce_api_unregistered_international_service' )
						);
					}
	...
	private function parse_etd( $etd ) {
		...
		$etd = str_replace(
			array( 'jam', 'hari' ),
			array( __( 'hours', 'ava-tokommerce' ), __( 'days', 'ava-tokommerce' ) ),
			strtolower( $etd )
		);

		if ( false === strpos( $etd, 'hours' ) && false === strpos( $etd, 'days' ) ) {
			$etd = trim( $etd ) . ' ' . __( 'days', 'ava-tokommerce' );
		}
